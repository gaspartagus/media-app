...
function navigate(chemin, arbre, controller) {
	//console.log(arbre,chemin)
	if(typeof arbre != 'object') return false;
	var keys = Object.keys(arbre);
	//console.log(clé,keys,keys.indexOf(clé))
	if(keys.indexOf(clé)>-1) {
		controller['']
	}
	for (var i = keys.length - 1; i >= 0; i--) {
		var essai = hasKey(clé, arbre[keys[i]], chemin.concat([keys[i]]) )
		var retour = [keys[i]];
		if(essai) return retour.concat(essai);
	};
	return false;
}

function hasKey(clé, arbre, chemin) {
	//console.log(arbre,chemin)
	if(typeof arbre != 'object') return false;
	var keys = Object.keys(arbre);
	//console.log(clé,keys,keys.indexOf(clé))
	if(keys.indexOf(clé)>-1) return [clé];
	for (var i = keys.length - 1; i >= 0; i--) {
		var essai = hasKey(clé, arbre[keys[i]], chemin.concat([keys[i]]) )
		var retour = [keys[i]];
		if(essai) return retour.concat(essai);
